        ...
        <div class="w-full bg-white shadow-md p-6 rounded-[5px]">
           <div class="text-[#0E1212] text-[20px] font-normal leading-[30px] mb-1">
            ...
            <table class="w-full border-collapse text-black">
                ...
                <tbody>
                    @php $jumlahSiswa = \App\Models\User::where('role', 'siswa')->count(); @endphp
                    @forelse ($kursus as $index => $item)
                        <tr class="{{ $loop->even ? 'bg-[#E8E6E6]' : 'bg-[#FFFFFF]' }}">
                            ...
                            <td class="px-4 py-2 border-b border-gray-300 text-center">{{ $jumlahSiswa }}</td>
                            <td class="px-4 py-2 border-b border-gray-300">
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.kursus.detail', $item->id) }}" class="w-[51px] h-[30px] bg-[#5EA3FD] rounded-[6px] flex items-center justify-center" title="Detail Kursus">
                                    ...
                                    <a href="{{ route('admin.kursus.edit', $item->id) }}" class="w-[51px] h-[30px] bg-[#FBB800] rounded-[6px] flex items-center justify-center" title="Edit Kursus">
                                    ...
                                    <form action="{{ route('admin.kursus.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kursus ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
